...
describe("Escrow", () => {
  ...
  beforeEach(async () => {
    ...
    const contractFactory: Escrow__factory = await ethers.getContractFactory(
      "Escrow"
    );
    ...
    const tx = await token.transfer(contract.address, BigInt(TOTAL_SUPPLY));
    await tx.wait();
  });
